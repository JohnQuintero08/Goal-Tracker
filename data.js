export const newGoal = {
  id: "",
  myGoal: "",
  numberTime: "",
  unitTime: "min",
  days: [{
          id: 'sunday',

          dayIsActive: false
      }, {
          id: 'monday',

          dayIsActive: false
      },
      {
          id: 'tuesday',

          dayIsActive: false
      },
      {
          id: 'wednesday',

          dayIsActive: false
      },
      {
          id: 'thursday',

          dayIsActive: false
      },
      {
          id: 'friday',

          dayIsActive: false
      },
      {
          id: 'saturday',

          dayIsActive: false
      },

  ],
  date: new Date(),
  task: [],
  percentage: 0,
}
export const myListOfGoals = [{
      id: 'SPT5610WTejco4tKB15Q7',
      myGoal: 'Create a new App',
      numberTime: '10',
      unitTime: 'min',
      days: [{
              id: 'sunday',

              dayIsActive: false
          }, {
              id: 'monday',

              dayIsActive: false
          },
          {
              id: 'tuesday',

              dayIsActive: false
          },
          {
              id: 'wednesday',

              dayIsActive: true
          },
          {
              id: 'thursday',

              dayIsActive: false
          },
          {
              id: 'friday',

              dayIsActive: true
          },
          {
              id: 'saturday',

              dayIsActive: true
          },

      ],
      date: new Date('2024-08-17T02:49:46.000Z'),
      task: [{
              task: 'Create the base layout',
              isDone: true,
              id: 'FzsNy6FkXUzSK-9sll5uW'
          },
          {
              task: 'Connect the auth to firebase',
              isDone: true,
              id: 'OdRKyWx4OzBF-XhSV5-bJ'
          },
          {
            task: 'Connect the google account button',
            isDone: false,
            id: 'j5IrKWx4OzBF-XhSV5U89'
        },
        {
            task: 'Connect the data base',
            isDone: false,
            id: '2h234Wx4OzBF-XhSV5-4is'
        },
        {
            task: 'Set up the edit button',
            isDone: false,
            id: 'Isda6Ux4OzBF-XhSV5U89'
        },
        {
            task: 'Set up the Calendar',
            isDone: true,
            id: 'Itashas4OzBF-XhSV5U89'
        },
        {
            task: 'Make it responsive',
            isDone: false,
            id: 'USiavais89asha7sfg-8d'
        },
      ],
      percentage: 42,
  },
  {
      id: 'RXOQsj0WTejco4tKB15QC',
      myGoal: 'Buy a car',
      numberTime: '1',
      unitTime: 'min',
      days: [{
              id: 'sunday',

              dayIsActive: true
          },
          {
              id: 'monday',

              dayIsActive: true
          },
          {
              id: 'tuesday',

              dayIsActive: false
          },
          {
              id: 'wednesday',

              dayIsActive: false
          },
          {
              id: 'thursday',

              dayIsActive: false
          },
          {
              id: 'friday',

              dayIsActive: false
          },
          {
              id: 'saturday',

              dayIsActive: true
          },
      ],
      date: new Date('2024-06-28T02:49:46.000Z'),
      task: [{
              task: 'Save 30 dollars',
              isDone: false,
              id: '7Dcaw3Wxfyne7IjvSgy2M'
          },
          {
              task: 'Not eating outside',
              isDone: false,
              id: 'uUkA3ro40YsylhGAIfoUo'
          }
      ],
      percentage: 0,
  },
  {
      id: 'rqxs5Ewh2SNVkpGgP37dL',
      myGoal: 'Going to Europe',
      numberTime: '3',
      unitTime: 'min',
      days: [{
              id: 'sunday',

              dayIsActive: false
          },
          {
              id: 'monday',

              dayIsActive: false
          },
          {
              id: 'tuesday',

              dayIsActive: false
          },
          {
              id: 'wednesday',

              dayIsActive: true
          },
          {
              id: 'thursday',

              dayIsActive: true
          },
          {
              id: 'friday',

              dayIsActive: true
          },
          {
              id: 'saturday',

              dayIsActive: false
          },
      ],
      date: new Date('2024-05-30T07:22:37.000Z'),
      task: [{
              task: 'Wining the lottery',
              id: 'XOW3G0LFj3JvphLyTBmod',
              isDone: false
          },
          {
              task: 'Saving $300 daily',
              id: 'r4KN-pDjf_Lc5dyjohvTJ',
              isDone: false
          },
          {
              task: 'Selling the car',
              id: 'noO87qmWd34SK8Anr4LeU',
              isDone: false
          },
          {
              task: 'Buy a life insurance',
              id: 't_xvLMExJIB18Q7KC9U-v',
              isDone: false
          }
      ],
      percentage: 0
  },
  {
      id: 'RX8gjs0WTejco4tKNj25O',
      myGoal: 'Learn React',
      numberTime: '20',
      unitTime: 'min',
      days: [{
              id: 'sunday',

              dayIsActive: false
          },
          {
              id: 'monday',

              dayIsActive: true
          },
          {
              id: 'tuesday',

              dayIsActive: true
          },
          {
              id: 'wednesday',

              dayIsActive: false
          },
          {
              id: 'thursday',

              dayIsActive: false
          },
          {
              id: 'friday',

              dayIsActive: false
          },
          {
              id: 'saturday',

              dayIsActive: false
          },
      ],
      date: new Date('2025-01-28T02:49:46.000Z'),
      task: [{
              task: 'Learn Props',
              isDone: false,
              id: '5Dgaw3Wxfyne7IjvSgy2M'
          },
          {
              task: 'Learn React Router',
              isDone: false,
              id: 'sTpA3ro40YsylhGAIfoUo'
          }
      ],
      percentage: 0,
  },
]